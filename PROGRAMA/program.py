...
class FacialRecognitionDB:
    ...
    def search_user_access(self, username):
        """Busca accesos de un usuario específico"""
        if not self.db_connection:
            return
    ...
    def run_system(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                username = input("Ingrese el nombre de usuario a buscar: ").strip()
                if username:
                    self.search_user_access(username)
            elif choice == "5":
                print("\n👥 USUARIOS REGISTRADOS:")
                if self.known_faces:
